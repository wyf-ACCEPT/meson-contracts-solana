...
impl Utils {
    ...
    pub const RELEASE_TYPE: [u8; 66] =
        *b"bytes32 Sign to release a swap on Meson (Testnet)address Recipient";

    pub const RELEASE_TYPE_TRON: [u8; 85] =
        *b"bytes32 Sign to release a swap on Mesonaddress Recipient (tron address in hex format)";
    ...
    pub fn amount_within_max(amount: u64) -> ProgramResult {
        if amount <= Self::MAX_SWAP_AMOUNT {
            Ok(())
        } else {
            Err(MesonError::SwapAmountOverMax.into())
        }
    }

    pub fn match_protocol_version(encoded_swap: [u8; 32]) -> ProgramResult {
        if Self::version_from(encoded_swap) == Self::MESON_PROTOCOL_VERSION {
            Ok(())
        } else {
            Err(MesonError::InvalidEncodedVersion.into())
        }
    }

    pub fn for_initial_chain(encoded_swap: [u8; 32]) -> ProgramResult {
        if Self::in_chain_from(encoded_swap) == Self::SHORT_COIN_TYPE {
            Ok(())
        } else {
            Err(MesonError::InChainMismatch.into())
        }
    }

    pub fn for_target_chain(encoded_swap: [u8; 32]) -> ProgramResult {
        if Self::out_chain_from(encoded_swap) == Self::SHORT_COIN_TYPE {
            Ok(())
        } else {
            Err(MesonError::OutChainMismatch.into())
        }
    }
    ...
    pub fn check_request_signature(
        ...
        if Self::in_chain_from(encoded_swap) == [0x00, 0xc3] {
            signing_data.extend_from_slice(if non_typed {
                &Self::TRON_SIGN_HEADER_33
            } else {
                &Self::TRON_SIGN_HEADER
            });
        ...
        if recovered == signer_eth_addr {
            ...
            Err(MesonError::InvalidSignature.into())
        }
    }
...
mod tests {
    ...
    fn test_amount_from() {
        let encoded_swap = [
            0x01, 0x00, 0x1d, 0xcd, 0x65, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x77,
            0x81, 0x5c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x4d, 0xcb, 0x98, 0x02, 0x7d,
            0x01, 0x02, 0xca, 0x21,
        ];
        assert_eq!(Utils::amount_from(encoded_swap), 500_000_000);

        let encoded_swap = [
            0x01, 0x01, 0x2a, 0x05, 0xf2, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x77,
            0x81, 0x5c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x4d, 0xcb, 0x98, 0x02, 0x7d,
            0x01, 0x02, 0xca, 0x21,
        ];
    ...
    fn test_fee_and_ts() {
        let encoded_swap = [
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x77,
            0x81, 0x5c, 0x00, 0x1d, 0xcd, 0x65, 0x00, 0x00, 0x63, 0x4d, 0xcb, 0x98, 0x02, 0x7d,
            0x01, 0x02, 0xca, 0x21,
        ];
        ...
        let encoded_swap = [
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x77,
            0x81, 0x5c, 0x01, 0x2a, 0x05, 0xf2, 0x00, 0x00, 0x63, 0x4d, 0xcb, 0x98, 0x02, 0x7d,
            0x01, 0x02, 0xca, 0x21,
        ];
    ...
    pub fn test_get_swap_id() {
        ...
        let initiator: [u8; 20] = [
            0x2e, 0xf8, 0xa5, 0x1f, 0x8f, 0xf1, 0x29, 0xdb, 0xb8, 0x74, 0xa0, 0xef, 0xb0, 0x21,
            0x70, 0x2f, 0x59, 0xc1, 0xb2, 0x11,
        ];
        ...
        assert_eq!(
            swap_id,
            [
                0xe3, 0xa8, 0x4c, 0xd4, 0x91, 0x2a, 0x01, 0x98, 0x9c, 0x6c, 0xd2, 0x4e, 0x41, 0xd3,
                0xd9, 0x4b, 0xaf, 0x14, 0x32, 0x42, 0xfb, 0xf1, 0xda, 0x26, 0xeb, 0x7e, 0xac, 0x08,
                0xc3, 0x47, 0xb6, 0x38
            ]
        );
    }
    ...
    fn test_eth_address_from_pubkey_64b() {
        let eth_pubkey = [
            ...
            0xe4, 0x35, 0x3e, 0x38, 0x2b, 0x48, 0x8d, 0x0c, 0x54, 0x3c, 0x50, 0x5e, 0xc4, 0x00,
            0x46, 0xaf, 0xbb, 0x25, 0x43, 0xba, 0x6b, 0xb5, 0x6c, 0xa4, 0xe2, 0x6d, 0xc6, 0xab,
    ...
    fn test_recover_eth_address() {
        let eth_addr = Utils::recover_eth_address(
            [
                0xea, 0x83, 0xcd, 0xcd, 0xd0, 0x6b, 0xf6, 0x1e, 0x41, 0x40, 0x54, 0x11, 0x5a, 0x55,
                0x1e, 0x23, 0x13, 0x37, 0x11, 0xd0, 0x50, 0x7d, 0xcb, 0xc0, 0x7a, 0x4b, 0xab, 0x7d,
                0xc4, 0x58, 0x19, 0x35,
            ],
            [
                0x2b, 0xd0, 0x3a, 0x0d, 0x8e, 0xdf, 0xcb, 0xe8, 0x2e, 0x56, 0xff, 0xed, 0xe5, 0xa9,
                0x4f, 0x49, 0x63, 0x5c, 0x80, 0x23, 0x64, 0x63, 0x0b, 0xc3, 0xbc, 0x9b, 0x17, 0xba,
                0x85, 0xba, 0xad, 0xfa, 0xb8, 0xb7, 0x33, 0x43, 0x7f, 0x0a, 0xd8, 0x97, 0xaa, 0x24,
                0x6d, 0x01, 0x11, 0x22, 0x57, 0x0c, 0x6c, 0x99, 0x43, 0xea, 0xd8, 0x62, 0x52, 0xd4,
                0xf1, 0x69, 0x52, 0x49, 0x53, 0x80, 0xa3, 0x1e,
            ],
        );
    ...
    fn test_check_request_signature() {
        ...
        let signature = [
            0xb3, 0x18, 0x4c, 0x25, 0x7c, 0xf9, 0x73, 0x06, 0x92, 0x50, 0xee, 0xfd, 0x84, 0x9a,
            0x74, 0xd2, 0x72, 0x50, 0xf8, 0x34, 0x3c, 0xbd, 0xa7, 0x61, 0x51, 0x91, 0x14, 0x9d,
            0xd3, 0xc1, 0xb6, 0x1d, 0x5d, 0x4e, 0x2b, 0x5e, 0xcc, 0x76, 0xa5, 0x9b, 0xaa, 0xbf,
            0x10, 0xa8, 0xd5, 0xd1, 0x16, 0xed, 0xb9, 0x5a, 0x5b, 0x20, 0x55, 0xb9, 0xb1, 0x9f,
            0x71, 0x52, 0x40, 0x96, 0x97, 0x5b, 0x29, 0xc2,
        ];
        let eth_addr = [
            0x2e, 0xf8, 0xa5, 0x1f, 0x8f, 0xf1, 0x29, 0xdb, 0xb8, 0x74, 0xa0, 0xef, 0xb0, 0x21,
            0x70, 0x2f, 0x59, 0xc1, 0xb2, 0x11,
        ];
        Utils::check_request_signature(encoded_swap, signature, eth_addr).unwrap();
    }
    ...
    fn test_check_release_signature() {
        ...
        let signature = [
            0x12, 0x05, 0x36, 0x1a, 0xab, 0xc8, 0x9e, 0x5b, 0x30, 0x59, 0x2a, 0x2c, 0x95, 0x59,
            0x2d, 0xdc, 0x12, 0x70, 0x50, 0x61, 0x0e, 0xfe, 0x92, 0xff, 0x64, 0x55, 0xc5, 0xcf,
            0xd4, 0x3b, 0xdd, 0x82, 0x58, 0x53, 0xed, 0xcf, 0x1f, 0xa7, 0x2f, 0x10, 0x99, 0x2b,
            0x46, 0x72, 0x1d, 0x17, 0xcb, 0x31, 0x91, 0xa8, 0x5c, 0xef, 0xd2, 0xf8, 0x32, 0x5b,
            0x1a, 0xc5, 0x9c, 0x7d, 0x49, 0x8f, 0xa2, 0x12,
        ];
        let eth_addr = [
            0x2e, 0xf8, 0xa5, 0x1f, 0x8f, 0xf1, 0x29, 0xdb, 0xb8, 0x74, 0xa0, 0xef, 0xb0, 0x21,
            0x70, 0x2f, 0x59, 0xc1, 0xb2, 0x11,
        ];
        Utils::check_release_signature(encoded_swap, recipient, signature, eth_addr).unwrap();
    }
}
